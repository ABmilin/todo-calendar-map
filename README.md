...
PCでの操作を想定していますが、スマホでも表示できるようになっています。
※スマホでは一部使えない機能があります。
...
### v0.2 (New)
- ✅ 「月のやらないことルール」作成 / 編集 / 有効無効
- ✅ ルールに基づく「警告」UI（ダッシュボード上部に表示）
  - ※ 判定ロジックは段階的に拡張予定（例：睡眠ブロック、移動バッファ 等）
...
### 4) 月のやらないことルール（v0.2）
1. 画面上部の「月のやらないことルール」からルールを追加
2. ルールの数値を編集して保存
3. 条件に当てはまる場合、上部に警告が出ます
...
### OpenStreetMap Tiles
本アプリの地図タイルは **OpenStreetMap Foundation のタイルサーバ** を使用しています。  
タイルサーバは公開サービスのため、**重い利用（大量アクセス）** は避ける必要があります。

### Reverse Geocoding (Nominatim)
地図クリック時の場所名取得に **Nominatim（OpenStreetMap）** を利用しています。  
利用ポリシー上、**大量アクセス禁止** や **識別可能な User-Agent の送信** 等の要件があります。

> もし公開デモが想定以上にアクセスされる場合は、商用プロバイダ利用や自前運用などへの切り替えが必要になります。
